...
if [ -z "$1" ]
then
	echo "\e[34mPlease provide Debians folder name 5249\e[0m"
	exit
fi

build_name=CI_DII_$1

echo "Downloading local *.deb files from Local PC $build_name"

scp -r mastan@10.145.169.54:/home/mastan/worksapce/git-pile/dii-tools/gr/CI_DII_BUILDS/$build_name .
